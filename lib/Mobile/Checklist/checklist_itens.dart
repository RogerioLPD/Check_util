...
class ChecklistTagScreen extends StatefulWidget {
  final String tag;
  final String agendamentoId;
  final String unidade;

  const ChecklistTagScreen({required this.tag,required this.unidade, required this.agendamentoId});

  @override
  State<ChecklistTagScreen> createState() => _ChecklistTagScreenState();
}

class _ChecklistTagScreenState extends State<ChecklistTagScreen> {
  ...
  void initState() {
    super.initState();
    _loadChecklistItems(widget.agendamentoId);
  }

  Future<void> _loadChecklistItems(String agendamentoId) async {
    final agendamentosProvider =
        Provider.of<AgendamentosProvider>(context, listen: false);
    await agendamentosProvider.fetchChecklistItemsPorTag(
        widget.tag, agendamentoId);
    setState(() {
      showChecklist = true;
    });
  }
  ...
  Future<void> _finalizeChecklist(Uint8List? signatureData) async {
    ...
    final agendamentosProvider =
        Provider.of<AgendamentosProvider>(context, listen: false);
    final checklists =
        agendamentosProvider.agendamentosComChecklist[widget.agendamentoId] ??
            [];
    ...
    final checklistFinalizado = {
      ...
      'itens': checklists
          .map((item) => {
                ...
                'status': item['status'] == 'naoConforme' ? 'Não Conforme' : item['status'],
    ...
    try {
      ...
      agendamentosProvider.removerAgendamento(widget.agendamentoId);
      setState(() {
        showChecklist = false;
          ...
          child: Column(
            children: [
              if (checklists.isEmpty)
                const Center(
                    child: Text("Nenhum item encontrado para esta tag.")),
